...
function runEffect<A>(effect: Effect.Effect<A, unknown, never>): Promise<A> {
...
export default function (pi: ExtensionAPI) {
  let actor: ReturnType<typeof createActor<typeof gatewayMachine>> | undefined;
    ...
    const relayTick = async () => {
      ...
      for (const channel of config.channels) {
        ...
        for (const message of messages) {
          if (!shouldRelayToTelegram(message, channel)) continue;
          if (message.receipts.some((receipt) => receipt.event === "telegram-relayed" && receipt.note === channel.id)) continue;
  ...
  pi.registerTool({
    ...
    async execute(_toolCallId, params, _signal, _onUpdate, ctx) {
      ...
      if (action === "publish") {
        if (!params.title) return text("title is required for publish", { ok: false });
      ...
      if (action === "claim") {
        if (!params.id) return text("id is required for claim", { ok: false });
