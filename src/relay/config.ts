...
export function readGatewayHomeConfig(home = gatewayHome()) {
  return readJsonFile<GatewayHomeConfig>(join(home, "config.json"), {});
}

export function readGatewayCredentials(home = gatewayHome()) {
  return readJsonFile<GatewayCredentials>(join(home, "credentials.json"), {});
}

export function routerConfigFromHome(home = gatewayHome()): TelegramRouterConfig {
