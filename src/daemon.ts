...
if (token && chatId) daemon.registerRelay(new TelegramRelay({ token, chatId }));
...
async function main() {
  ...
  if (command === "route") {
    ...
    if (!project || !title) throw new Error("usage: pi-gateway-daemon route <project> <title> [body]");
    await Effect.runPromise(daemon.start());
  ...
  if (command === "claim") {
    ...
    if (!project || !messageId) throw new Error("usage: pi-gateway-daemon claim <project> <message-id>");
    await Effect.runPromise(daemon.start());
