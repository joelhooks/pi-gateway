...
export function createTelegramRouter(config) {
    ...
    function defaultWakeCommand(project) {
    ...
    function wakeProjectAgent(project) {
        if (project.wakeCommand === false)
            return { ok: false, error: "agent wake disabled" };
    ...
    function spawnProjectWorker(project, messageId, threadId) {
    ...
    async function handle(text, threadId = "telegram") {
        ...
        if (!parsed) {
            ...
            if (!agent)
                spawnProjectWorker(selected, message.id, threadId);
